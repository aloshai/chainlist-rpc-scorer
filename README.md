...
A GitHub Action probes every RPC listed on [Chainlist](https://chainlist.org), measures latency, liveness, block freshness, and rate-limit tolerance for each endpoint, and commits a ranked list into this repository. The pipeline has no runtime service and no database; the output is read directly over HTTP.
